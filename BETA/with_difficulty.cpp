...
if(easyMode != true){
    ...
    // --- Easy mode: pick a completely random available square ---
    char* squares_arr[9] = {
    ...
    int count = 0;
    for (int i = 0; i < 9; i++) {
        if (*squares_arr[i] == ' ') options[count++] = i;
    }

    if (count > 0) {
        int pick = options[rand() % count];
        *squares_arr[pick] = 'O';
    }
}
...
int main(){
    ...
    if(aiChoice == "y"){
        ai = true;
        std::cout << "Would you like to play against ai in easy mode (y/n): ";
        std::cin >> response;
        if(response == 'y'){
            easyMode = true;
        }
